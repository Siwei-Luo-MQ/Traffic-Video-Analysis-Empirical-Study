...
def message_template(img_list):
    ...
    Next, I need to identify the traffic participants their behaviors~
...
def evaluation(dataset, model, processor, args):
    ...
    for instance in dataset:
        ...
        sampled_frames = sample_frames(frames_path, args.sampled_num)
        print('=========================================')
        ...
        print('-----------------------------------------')
        print('LLM outputs using sampled frames: ')
        print(model_output)
        print('-----------------------------------------')
        llm_fmt_res = extract_accident_label(model_output)
        print('Extracted results: ')
        print(llm_fmt_res)
        if llm_fmt_res['Accident'] == 'No':
            # if no accident detected from slected frames, we assume no accidents
            flag = 0 # prediction result
            predict_target.append(flag)
            print('-----------------------------------------')
            if accident_label == 1: # wrongly predicted
                print('True label: Yes')
                frame_idx = init_frame + 250
                acc_counter += 1
                frame_diff = (frame_idx - init_frame) ** 2 + frame_diff
            else:
                print('True label: No') # correctly predicted
        else: # -> llm_fmt_res['Accident'] == 'Yes'
            # frame-level detection
            # prepare message
            print('++++++Accident detected! Doing frame-level detection: ++++++')
